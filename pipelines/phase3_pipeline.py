...
from core.column_roles import build_column_roles
...
def run_phase3_intel(df: pd.DataFrame, schema: dict[str, str], state: AnalysisState) -> None:
    ...
    column_roles = build_column_roles(cols_meta)
    ...
    anomaly_bundle = build_anomaly_intelligence(df_coerced, schema, column_roles=column_roles)
...
def rerun_validation_intel(df: pd.DataFrame, schema: dict[str, str], state: AnalysisState) -> None:
    """Re-run validation gate + legacy manager only (after role confirm in Step 3)."""
    df_coerced = normalize_schema(df, schema)
    cols_meta = (
        state.semantic_profile.get("columns") if isinstance(state.semantic_profile.get("columns"), dict) else {}
    )
    ...
    archetypes = dataset_ctx.get("archetypes") if isinstance(dataset_ctx, dict) else None
    column_roles = build_column_roles(cols_meta)
