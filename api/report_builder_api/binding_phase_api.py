...
def _load_gold_subpackage_blueprint(template_id: str) -> dict[str, Any] | None:
    """Discover a bundled gold-standard blueprint by template id (subdirectory packages).

    Each enterprise gold template ships as ``gold_standard/<slug>/<slug>.template.blueprint.json``
    (slug = template_id without the ``tpl_`` prefix). This lets built-in subdirectory
    templates bind with zero config, mirroring how the generate-phase loads their AST.
    """
    if not template_id:
        return None
    slug = template_id.replace("tpl_", "")
    candidates = [
        _GOLD_DIR / slug / f"{slug}.template.blueprint.json",
        _GOLD_DIR / f"{slug}.template.blueprint.json",
        _GOLD_DIR / template_id / f"{template_id}.template.blueprint.json",
    ]
    for path in candidates:
        if path.exists():
            try:
                bp = json.loads(path.read_text(encoding="utf-8-sig"))
                if isinstance(bp, dict) and bp.get("entities"):
                    logger.info("[binding-phase] gold blueprint loaded: %s", path.name)
                    return bp
            except Exception as exc:  # noqa: BLE001
                logger.warning("[binding-phase] gold blueprint read failed %s: %s", path, exc)
    return None
...
async def _resolve_blueprint(template_id: str, blueprint_file: Optional[UploadFile]) -> dict[str, Any]:
    ...
    # 3b. Bundled gold subdirectory packages (enterprise templates by id).
    sub_bp = _load_gold_subpackage_blueprint(template_id)
