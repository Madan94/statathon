...
def _pass_dump_entity(entity: dict[str, Any]) -> dict[str, Any]:
    """Serialize an entity to a binder-ready dict, preserving evidence + runtime refs.

    Keeps sourceRefs (with regionRef/physicalColumn/tableId/bbox/headerPath/confidence),
    runtimeTraceRefs, unit and valueDomain intact so the binder can trust S1/S2 review.
    """
    etype = entity.get("entityType") or entity.get("entityType_hint") or "dimension"
    dumped: dict[str, Any] = {
        "entityId": entity.get("entityId") or "",
        "name": entity.get("name") or entity.get("canonicalName") or "",
        "canonicalName": entity.get("canonicalName") or entity.get("name") or "",
        "entityType": etype,
        "entityType_hint": entity.get("entityType_hint") or etype,
        "aliases": list(entity.get("aliases") or []),
        "unit": entity.get("unit"),
        "aggregation": entity.get("aggregation"),
        "valueDomain": entity.get("valueDomain"),
        "pages": list(entity.get("pages") or []),
        "confidence": entity.get("confidence"),
        "sourceRefs": [dict(r) for r in (entity.get("sourceRefs") or []) if isinstance(r, dict)],
        "runtimeTraceRefs": [dict(r) if isinstance(r, dict) else r for r in (entity.get("runtimeTraceRefs") or [])],
    }
    return dumped


def _build_template_semantic_graph(state: dict[str, Any]) -> dict[str, Any]:
    """Build the canonical template semantic graph from extraction state.

    Threads every entity's ``sourceRefs`` into a global ``evidenceIndex`` (one record
    per source ref, distinct bboxes kept separate) and links each entity to its
    evidence via ``evidenceRefs``. This is the single source of binder evidence.
    """
    all_entities = state.get("all_entities") or []
    evidence_index: list[dict[str, Any]] = []
    _ev_seq = [0]

    def _next_ev_id() -> str:
        _ev_seq[0] += 1
        return f"ev_{_ev_seq[0]:04d}"
    ...
    for ent in all_entities:
        if not isinstance(ent, dict):
            continue
        refs = [r for r in (ent.get("sourceRefs") or []) if isinstance(r, dict)]
        ...
        for ref in refs:
            ...
            record = {"evidenceId": eid, "entityId": ent.get("entityId") or "", **ref}
            evidence_index.append(record)
        etype = ent.get("entityType") or ent.get("entityType_hint") or "dimension"
        graph_entities.append({
            "entityId": ent.get("entityId") or "",
            "name": ent.get("name") or ent.get("canonicalName") or "",
            "entityType": etype,
            "unit": ent.get("unit"),
            "pages": list(ent.get("pages") or []),
            "evidenceRefs": evidence_refs,
            "sourceRefs": [dict(r) for r in refs],
        })

    chapters = [dict(c) for c in (state.get("chapters") or []) if isinstance(c, dict)]
    tables = [dict(t) for t in (state.get("table_structures") or []) if isinstance(t, dict)]
    relationships = [dict(r) for r in (state.get("entity_relationships") or []) if isinstance(r, dict)]

    return {
        "$schema": "bharatstat/template-semantic-graph/v1",
        "document": {
            "title": state.get("title") or "",
            "pageCount": state.get("page_count") or 0,
        },
        ...
        "diagnostics": {
            ...
            "entitiesWithSourceRefs": entities_with_refs,
            "entitiesMissingSourceRefs": len(graph_entities) - entities_with_refs,
            "evidenceCount": len(evidence_index),
...
def _question_local_evidence(
    question: dict[str, Any],
    entities: list[dict[str, Any]],
    chapters: list[dict[str, Any]],
) -> tuple[bool, dict[str, Any]]:
    """Whether a question's measure has evidence local to the question's section.

    Returns ``(ok, evidence)``. A measure entity referenced in the question intent must
    have a page within the same chapter/section page range as the question. A measure
    found only in another section is rejected (``out_of_section_measure``) so the binder
    never wires a question to a measure that does not appear where it is asked.
    """
    intent = str(question.get("intent") or question.get("questionText") or "").lower()
    q_page = question.get("page")

    # Resolve the question's section page range from the chapters.
    section_range: tuple[int, int] | None = None
    for ch in chapters or []:
        pr = ch.get("pageRange") or []
        if len(pr) == 2 and q_page is not None and pr[0] <= q_page <= pr[1]:
            section_range = (int(pr[0]), int(pr[1]))
            break

    # Find measure entities whose name/alias is mentioned in the intent.
    matched_measures: list[dict[str, Any]] = []
    for ent in entities or []:
        if (ent.get("entityType") or ent.get("entityType_hint")) != "measure":
            continue
        names = [str(ent.get("name") or ent.get("canonicalName") or "")] + [str(a) for a in (ent.get("aliases") or [])]
        if any(n and n.lower() in intent for n in names):
            matched_measures.append(ent)

    if not matched_measures:
        return False, {"reason": "no_measure_in_intent", "questionPage": q_page}

    if section_range is not None:
        lo, hi = section_range
        for ent in matched_measures:
            pages = [p for p in (ent.get("pages") or []) if isinstance(p, int)]
            if any(lo <= p <= hi for p in pages):
                return True, {
                    "reason": "local_measure_evidence",
                    "measureEntityId": ent.get("entityId"),
                    "sectionRange": [lo, hi],
                }
        return False, {
            "reason": "out_of_section_measure",
            "measureEntityId": matched_measures[0].get("entityId"),
            "sectionRange": [section_range[0], section_range[1]],
        }

    # No section context: accept if any matched measure has the question's page.
    for ent in matched_measures:
        if q_page in (ent.get("pages") or []):
            return True, {"reason": "local_measure_evidence", "measureEntityId": ent.get("entityId")}
