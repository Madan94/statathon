...
| R4 customization | ✅ done | `pending-hash` | profile.py: TemplateProfile (full author defaults) + ReportOverrides (sparse) + deep_merge/effective_profile (override wins, perQuestion per-qid deep-merge) + apply_profile (reorder/filter-questions/swap chartType/table format + metadata stamp) + render_flags; render_html/render_pdf threaded number_system (additive, default indian); API GET/PUT profile, GET/PATCH overrides, POST /render?format=html|pdf; dashboard lib/report/profile.ts (client applyProfile parity) + CustomizePanel + SectionReorder + api {get,put}Profile/{get,patch}Overrides/customRenderUrl + preview page live customization; tests/test_render_profile.py 18 tests; gate 169 green; tsc clean |
| R5 edit+audit | pending | — | edit.py apply_edit, override w/ reason+audit, versioned report.v{n}; API edit/versions; EditableField |
