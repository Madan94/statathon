...
| R2 weasyprint pdf | ✅ done | `156113c` | render/pdf.py engine dispatch + pdf_available(); render_pdf chrome-on default; GET /report.pdf endpoint (503 when engine absent, 400 unknown engine, 404 no report); dashboard reportPdfUrl(); 9 tests (+1 skip w/o WeasyPrint); gate 151 green. NOTE: exporter.py retirement DEFERRED until WeasyPrint verified on a host with native libs |
| R3 react preview | ✅ done | `26e2cb4` | dashboard: lib/report/{types,format,chartOption}.ts (format parity w/ numbers.py — Indian grouping/percent/bilingual loc); render/{ReportChart(ECharts, lazy import + ResizeObserver),ReportTable(col groups/measure fmt/subtotal bold/footnote markers/sticky head),ProvenanceDrawer(rowIds+evidence chain),ReportPreview(walks sections by order)}; route app/report-builder/preview?tid=&sig= (Interactive/Server-HTML tabs + locale toggle + PDF); binding page Step-3 wired "Live preview"+"Download PDF"; echarts@^5.5.0; tsc gate clean (only pre-existing clusterUtils.test.ts errors) |
| R4 customization | ✅ done | `c821cd4` | profile.py: TemplateProfile (full author defaults) + ReportOverrides (sparse) + deep_merge/effective_profile (override wins, perQuestion per-qid deep-merge) + apply_profile (reorder/filter-questions/swap chartType/table format + metadata stamp) + render_flags; render_html/render_pdf threaded number_system (additive, default indian); API GET/PUT profile, GET/PATCH overrides, POST /render?format=html|pdf; dashboard lib/report/profile.ts (client applyProfile parity) + CustomizePanel + SectionReorder + api {get,put}Profile/{get,patch}Overrides/customRenderUrl + preview page live customization; tests/test_render_profile.py 18 tests; gate 169 green; tsc clean |
| R5 edit+audit | ✅ done | `7b5772d` | edit.py: apply_edit (prose re-validated via narrator.validate_numbers w/ value+gap allowed-set → 400 on hallucinated number; number override requires reason + flags element overridden + appends auditAST.humanReview.edits[]; free text caption/footnote/header/title), bump_version/current_version; API POST /edit (validate→snapshot v1→write v{n}→update current+html), GET /versions, GET /report?version=n; dashboard EditableField (inline text + number override modal w/ reason + overridden badge) + VersionHistory + ReportPreview/ReportTable threaded editable/onEdit + preview page Edit toggle & version view; tests/test_render_edit.py 16 tests; gate 185 green; tsc clean |
| R6 latex/tectonic | ✅ done | `pending-hash` | render/latex.py: tectonic_available() (shutil.which), render_latex (.tex: titlepage cover/maketitle, tableofcontents, \section per section, longtable+booktabs w/ \endhead repeating header, measure cells format_value+escaped, subtotal bold, footnotes; LaTeX-escaping; charts as caption placeholders — no plot toolchain dep), compile_pdf_tectonic (subprocess tectonic --outdir, graceful None), render_pdf_latex; pdf.py engine=latex routes here (number_system threaded); API report.pdf?engine=latex (already wired); dashboard preview "PDF (LaTeX)" button; tests/test_render_latex.py 9 tests (+1 skip w/o Tectonic); full gate 194 passed/2 skipped (WeasyPrint+Tectonic); default WeasyPrint path untouched; tsc clean |
